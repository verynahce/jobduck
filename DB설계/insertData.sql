...
INSERT INTO RESUME (
    ...
    1,
    (SELECT NVL(MAX(RESUME_IDX)+1,1) FROM RESUME),
    '춘씨 헛소리말고 짐이나 날라',
    '자기소개입니다.'
);
...
INSERT INTO RESUME_SKILLS (
    ...
    (SELECT NVL(MAX(RESUME_SKILL_IDX)+1,1) FROM RESUME_SKILLS),
    1,
    10
);
...
INSERT INTO COMPANIES (
    ...
    (SELECT NVL(MAX(COMPANY_IDX)+1,0) FROM COMPANIES),
    'qwe123',
    '1234',
    '그린아카데미',
    '123123451231233',
...
INSERT INTO POST (
    ...
    (SELECT NVL(MAX(POST_IDX)+1,1) FROM POST),
    1,
    '인재 모집합니다.',
    '인재 찾았습니다.'
);
...
INSERT INTO POST_BENEFITS (
    ...
    (SELECT NVL(MAX(POST_BENEFIT_IDX)+1,1) FROM POST_BENEFITS),
    10,
    1
);
...
SELECT *
FROM APPLICATION A, USERS U, POST P, COMPANIES C, RESUME R
WHERE A.USER_IDX = U.USER_IDX
      AND P.POST_IDX = A.POST_IDX
      AND P.COMPANY_IDX = C.COMPANY_IDX
      AND U.USER_IDX = R.USER_IDX(+) ;
